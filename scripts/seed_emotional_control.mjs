/**
 * SoulThread — Emotional Control Posts Seeder
 * Creates a MindsetGuide user profile and publishes 30 emotional control posts.
 */
...
const USER_PASS = "Mindset@Guide2024";
...
async function ensureUser() {
  let uid;
  try {
    const cred = await createUserWithEmailAndPassword(auth, USER_EMAIL, USER_PASS);
    uid = cred.user.uid;
    console.log("✅ Created new Auth user:", uid);
  } catch (err) {
    if (err.code === "auth/email-already-in-use") {
      const cred = await signInWithEmailAndPassword(auth, USER_EMAIL, USER_PASS);
      uid = cred.user.uid;
      console.log("ℹ️ User exists, signed in:", uid);
    } else {
      throw err;
    }
  }
...
async function main() {
  console.log("🚀 Starting Emotional Control Seeder...");
  ...
  console.log("🎉 All 30 posts published!");
  process.exit(0);
}
